...
""",
    ...
    def _run_pdf_svg_pipeline(self, *, tex_path: Path, pdf_path: Path, svg_path: Path, work_dir: Path, compiler: str) -> None:
        subprocess.run(
            [
                compiler,
    ...
    def _run_dvi_svg_pipeline(self, *, tex_path: Path, svg_path: Path, work_dir: Path) -> None:
        ...
        subprocess.run(
            ["dvisvgm", "-n", "--bbox=preview", "-o", str(svg_path), str(dvi_path)],
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
    ...
    def _compile_tex_to_svg(self, *, tex_path: Path, pdf_path: Path, svg_path: Path, work_dir: Path, compiler: str) -> None:
        # dvisvgm 3.2.1 drops PDF ExtGState transparency from TikZ output.
        # The DVI specials generated by latex preserve it as SVG fill/stroke opacity.
        if compiler == "pdflatex":
            try:
                self._run_dvi_svg_pipeline(tex_path=tex_path, svg_path=svg_path, work_dir=work_dir)
                return
            except (subprocess.CalledProcessError, FileNotFoundError):
                pass

        self._run_pdf_svg_pipeline(
            tex_path=tex_path,
            pdf_path=pdf_path,
            svg_path=svg_path,
            work_dir=work_dir,
    ...
    def generate(self):
        ...
        # If the user pasted a full TikZ/Feynman environment while in formula mode,
        # route rendering through the TikZ pipeline to avoid formula-box cropping.
        if mode == "formula" and re.search(r"\\begin\{(?:tikzpicture|feynman)\}", formula):
            mode = "tikz"
        ...
        if mode == "tikz":
            ...
            tex = (
                # Use preview-based cropping for TikZ content: it's generally
                # more robust on complex diagrams than standalone's tikz crop.
                "\\documentclass[preview,border=72pt]{standalone}\n"
                f"{preamble}\n"
                "\\begin{document}\n"
                f"{body}\n"
                "\\end{document}\n"
            ...
            # Cache-bust TikZ renders when the TikZ template/bounding strategy changes.
            key = self._render_cache_key(compiler=compiler, mode="tikz_v3", preamble=preamble, body=body)
        ...
        key = self._render_cache_key(compiler=compiler, mode="formula_v2", preamble=preamble, body=body)
        if key == self._last_render_key and self.svg_data:
            self.status.config(text="Already up to date")
            return
        if self._cache_try_load(key):
